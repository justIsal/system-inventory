...
export const ProductForm: React.FC<ProductFormProps> = ({
      ...
      return {
        ...
        variants:
          initialData.variants?.map((v) => ({
            variant_id: (v as unknown as Record<string, unknown>).variant_id as number | undefined, // Map the backend ID for updates
            sku: v.sku,
            ...
            specifications: (v as unknown as Record<string, unknown>).specifications as
  ...
  const form = useForm({
    ...
    // @ts-expect-error - Mismatched generic FormOptions type
  ...
  const calculateInitialVariantOptions = (): VariantOption[] => {
    if (!initialData || !initialData.has_variants || !initialData.variants)
      return [
    ...
    initialData.variants.forEach((v) => {
      const specs = (v as unknown as Record<string, unknown>).specifications || {};
  ...
  const [variantOptions, setVariantOptions] = useState<VariantOption[]>(
    calculateInitialVariantOptions(),
  );
  ...
  const [nestedNotification, setNestedNotification] = useState<{
    type: 'success' | 'error';
    message: string;
  } | null>(null);
  ...
  useEffect(() => {
    Promise.all([apiClient.get(API_ROUTES.CATEGORIES.BASE), supplierService.getSuppliers(1, 100)])
      .then(([catRes, supRes]) => {
  ...
  useEffect(() => {
    ...
    if (validOptions.length === 0 && !initialData) {
      ...
      setGeneratedTable(
        initialData.variants.map((v) => ({
          variant_id: (v as unknown as Record<string, unknown>).variant_id, // Map hidden ID
          ...
          specs: (v as unknown as Record<string, unknown>).specifications || {},
        })) as unknown as GeneratedVariant[],
      );
      return;
    }

    const combine = (
      acc: { comb: string; specs: Record<string, string> }[],
      idx: number,
    ): { comb: string; specs: Record<string, string> }[] => {
      if (idx === validOptions.length) return acc;
      const currentOpt = validOptions[idx];
      if (acc.length === 0)
        return combine(
          currentOpt.values.map((v) => ({ comb: v, specs: { [currentOpt.name]: v } })),
          idx + 1,
        );
    ...
    setGeneratedTable((prevTable) => {
      const newTable = combinations.map((c) => {
        const existing = prevTable.find((gt) => gt.combination === c.comb);
        return existing
          ? existing
          : {
              combination: c.comb,
      ...
      // Sync to Form Store safely within the update loop considering the new variants
      // Set timeout ensures form context is updated asynchronously after the react state cycle
      setTimeout(() => {
        form.setFieldValue(
          'variants',
          newTable.map((t) => ({
            ...
            unit: globalUnit,
            name: t.combination,
            specifications: (t as Record<string, unknown>).specs as Record<string, unknown>,
          })),
        );
      }, 0);

      return newTable as unknown as GeneratedVariant[];
    });
  }, [variantOptions, hasVariants, globalUnit, initialData, form]);

  // Handlers
  const handleVariantTableChange = (index: number, field: keyof GeneratedVariant, val: unknown) => {
    const newTable = [...generatedTable];
    (newTable[index] as unknown as Record<string, unknown>)[field] = val;
  ...
  const handleCreateSupplier = async (
    data: Parameters<typeof supplierService.createSupplier>[0],
  ) => {
    ...
    try {
      ...
      setNestedNotification({ type: 'error', message: err.response?.data?.message || 'Gagal' });
    } finally {
        ...
        <div className="space-y-8 bg-slate-50/50 -m-6 p-6">
                ...
                {(field) => (
                  <div className="col-span-2">
                    ...
                    <Input
                      ...
                      placeholder="Misal: Kemeja Flanel Erigo"
                      error={
                        field.state.meta.errors
                          ? field.state.meta.errors
                              .map((err: unknown) =>
                                typeof err === 'string'
                                  ? err
                                  : (err as { message?: string })?.message || 'Wajib diisi',
                              )
                              .join(', ')
                          : undefined
                      }
                ...
                {(field) => (
                  <div>
                    ...
                    <select
                      ...
                      className="w-full p-2.5 text-sm border border-slate-300 rounded-lg outline-none max-h-10"
                    >
                ...
                {(field) => (
                  <div className="col-span-2">
                    ...
                    <textarea
                      ...
                      className="w-full p-2.5 text-sm border border-slate-300 rounded-lg outline-none resize-none"
                    ></textarea>
                  </div>
                )}
              </form.Field>
          ...
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-4">
            <h3 className="text-sm font-bold text-slate-800 border-b pb-2">
              2. Pengaturan Operasional & Supplier
            </h3>
            ...
            <form.Field name="default_supplier_id">
              {(field) => (
                <div className="mb-4">
                  <label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">
                    Supplier Utama (Default PO)
                  </label>
                  <div className="flex gap-2">
                    <select
                      value={field.state.value || ''}
                      onChange={(e) => field.handleChange(Number(e.target.value))}
                      className="flex-1 p-2.5 text-sm border border-slate-300 rounded-lg outline-none"
                    >
                    ...
                    <button
                      ...
                      className="px-3 border border-blue-200 text-blue-600 rounded-lg hover:bg-blue-50 text-sm font-bold flex items-center gap-1"
                    >
                ...
                {(field) => (
                  <label className="flex items-center space-x-3 cursor-pointer p-2 hover:bg-slate-50 rounded-lg">
                    ...
                    <span className="text-sm font-semibold text-slate-700">
                      Wajib Lacak Tanggal Kadaluarsa (Expired Date)
                    </span>
                  </label>
                )}
              </form.Field>
              <form.Field name="track_sn">
                {(field) => (
                  <label className="flex items-center space-x-3 cursor-pointer p-2 hover:bg-slate-50 rounded-lg">
                    ...
                    />
                    <span className="text-sm font-semibold text-slate-700">
                      Wajib Scan Serial Number (SN/IMEI)
                    </span>
                  </label>
                )}
              </form.Field>
                  ...
                  <label className="flex items-center space-x-3 cursor-pointer p-3 bg-blue-50 border border-blue-100 rounded-lg">
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-blue-700">
                        Kemeja berbagai ukuran/warna, dll.
                      </span>
                    </div>
                  </label>
                )}
          ...
          {!hasVariants && (
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-5 animate-fade-in">
              <h3 className="text-sm font-bold text-slate-800 border-b pb-2">
                3. Detail SKU & Harga (Produk Tunggal)
              </h3>
                  ...
                  {(field) => (
                    <div>
                      ...
                      <select
                        value={field.state.value}
                        onChange={(e) => field.handleChange(e.target.value)}
                        className="w-full p-2.5 text-sm border border-slate-300 rounded-lg outline-none bg-white max-h-10"
                      >
                        <option>Pcs</option>
                        <option>Box</option>
                        <option>Set</option>
              ...
              <div className="pt-4 border-t border-slate-100">
                ...
                {specs.map((spec, idx) => (
                  <div key={idx} className="flex gap-2 mb-2">
                    <input
                      type="text"
                      placeholder="Nama Label (Misal: Bahan)"
                      value={spec.key}
                      onChange={(e) => {
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        const updatedSpecs = newSpecs.reduce(
                          (acc, curr) => {
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (specs.length > 1) {
                          ...
                          const curVars = form.getFieldValue('variants');
                          if (curVars[0]) {
                            curVars[0].specifications = newSpecs.reduce(
                              (acc, curr) => {
                                if (curr.key.trim() !== '') acc[curr.key] = curr.value;
                                return acc;
                              },
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-5 animate-fade-in border-l-4 border-l-blue-500">
              ...
              <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 space-y-3">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const newVars = form
                        ...
                        .map((v) => ({ ...(v as object), unit: e.target.value }));
                      // @ts-expect-error - Expected inference drift from generic overrides
                  ...
                  <div key={opt.id} className="flex gap-3 items-start">
                    ...
                    <div className="flex-1">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const n = [...variantOptions];
                          n[idx].values = e.target.value;
                          setVariantOptions(n);
                        }}
                        placeholder="Hitam, Putih, Merah"
                        className="w-full p-2.5 text-sm border border-slate-300 rounded-lg outline-none font-mono text-blue-600 focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                  </div>
                ))}
                <button
                  ...
                  className="text-xs font-bold text-slate-600 bg-white border border-slate-300 px-3 py-1.5 rounded"
                >
                  + Tambah Tipe
                </button>
              </div>
                        ...
                        <tr key={i} className="hover:bg-slate-50">
                          ...
                          <td className="p-2">
                            <input
                              type="text"
                              value={v.sku}
                              onChange={(e) => handleVariantTableChange(i, 'sku', e.target.value)}
                              placeholder={`SKU-${i + 1}`}
                              className="w-full p-2 text-xs border border-slate-300 rounded uppercase font-mono"
                            />
                          </td>
                          <td className="p-2">
                            <input
                              ...
                              value={v.barcode}
                              onChange={(e) =>
                                handleVariantTableChange(i, 'barcode', e.target.value)
                              }
                              placeholder="Barcode"
        ...
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-100 animate-in slide-in-from-top-4 fade-in duration-300 min-w-[300px] shadow-lg rounded-xl">
          ...
          >
            ...
            <button
              ...
              className="ml-4 p-1 rounded-full hover:bg-black/5"
            >
