...
function RouteComponent() {
  ...
  if (!isOpen)
    return (
      <button onClick={() => setIsOpen(true)} className="p-4 bg-blue-600 text-white rounded">
    ...
    <div className="fixed inset-0 bg-slate-900/50 flex justify-end z-50 font-sans">
      ...
      <div className="w-full max-w-4xl bg-white h-full shadow-2xl flex flex-col animate-slide-in-right">
        ...
        <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium">
              Buat identitas produk baru ke dalam katalog database.
            </p>
          </div>
          <button
            ...
            <div className="mb-4">
              ...
              <div className="border-2 border-dashed border-slate-300 rounded-xl p-8 flex flex-col items-center justify-center text-slate-500 hover:bg-blue-50 hover:border-blue-400 hover:text-blue-600 transition-colors cursor-pointer group">
                <svg
                  ...
                  className="h-10 w-10 mb-2 text-slate-400 group-hover:text-blue-500 transition-colors"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
                ...
                <span className="text-[10px] uppercase tracking-wider mt-1 text-slate-400">
                  PNG, JPG up to 5MB
                </span>
              </div>
            </div>
          ...
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-3">
            ...
            <label className="flex items-center space-x-3 cursor-pointer p-2 hover:bg-slate-50 rounded-lg">
              ...
              <span className="text-sm font-semibold text-slate-700">
                Wajib Lacak Tanggal Kadaluarsa (Expired Date) saat Inbound
              </span>
            </label>
            <label className="flex items-center space-x-3 cursor-pointer p-2 hover:bg-slate-50 rounded-lg">
              <input
              ...
              />
              <span className="text-sm font-semibold text-slate-700">
                Wajib Scan Serial Number (SN/IMEI) saat Inbound
              </span>
            </label>
              ...
              <label className="flex items-center space-x-3 cursor-pointer p-3 bg-blue-50 border border-blue-100 rounded-lg">
                ...
                <div>
                  ...
                  <span className="text-[10px] text-blue-700">
                    Pilih ini jika produk memiliki warna, ukuran, atau tipe yang berbeda-beda.
                  </span>
                </div>
              </label>
            </div>
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-5 animate-fade-in">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="KEM-FLN-001"
                    className="w-full p-2.5 text-sm border border-slate-300 rounded-lg outline-none uppercase font-mono"
                  />
                </div>
                <div>
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1 block">
                  ...
                  <input
                    type="text"
                    placeholder="899123..."
                    className="w-full p-2.5 text-sm border border-slate-300 rounded-lg outline-none font-mono"
                  />
                </div>
                <div>
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1 block">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    placeholder="10"
                    className="w-full p-2.5 text-sm border border-slate-300 rounded-lg outline-none"
                  />
                </div>
                <div>
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1 block">
              ...
              <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 space-y-3">
                <div className="mb-4 pb-4 border-b border-slate-200">
                  ...
                  <p className="text-[10px] text-slate-500 mt-1">
                    Berlaku untuk semua SKU Anak di bawah ini.
                  </p>
                </div>
                ...
                <button
                  ...
                  className="text-xs font-bold text-slate-600 bg-white border border-slate-300 px-3 py-1.5 rounded hover:bg-slate-100 transition-colors"
                >
                  + Tambah Tipe Varian
                </button>
              </div>
                ...
                <table className="w-full text-left text-sm whitespace-nowrap">
                  <thead className="bg-slate-800 text-white">
                    <tr>
                      ...
                      <th className="p-3 text-[10px] uppercase font-bold tracking-wider w-32">
                        SKU Anak *
                      </th>
                      <th className="p-3 text-[10px] uppercase font-bold tracking-wider w-32">
                        Barcode
                      </th>
                      <th className="p-3 text-[10px] uppercase font-bold tracking-wider w-24">
                        Harga Beli
                      </th>
                      <th className="p-3 text-[10px] uppercase font-bold tracking-wider w-24">
                        Harga Jual
                      </th>
                      <th className="p-3 text-[10px] uppercase font-bold tracking-wider w-20">
                        Berat(gr)
                      </th>
                      <th className="p-3 text-[10px] uppercase font-bold tracking-wider w-20">
                        Min Stok
                    ...
                    {generatedVariants.length === 0 ? (
                      <tr>
                        <td colSpan={7} className="p-4 text-center text-slate-400 text-xs italic">
                          Ketik nilai varian di atas untuk menghasilkan daftar SKU otomatis...
                        </td>
                      </tr>
                    ) : (
                      generatedVariants.map((v: unknown, i: number) => {
                        const variant = v as { combination: string };
                        return (
                          <tr key={i} className="hover:bg-slate-50 transition-colors">
                            <td className="p-3 font-bold text-slate-700">{variant.combination}</td>
                            <td className="p-2">
                              <input
                                type="text"
                                placeholder={`SKU-${i + 1}`}
                                className="w-full p-2 text-xs border border-slate-300 rounded outline-none uppercase font-mono focus:border-blue-500"
                              />
                            </td>
                            <td className="p-2">
                              <input
                                type="text"
                                placeholder="Barcode"
                                className="w-full p-2 text-xs border border-slate-300 rounded outline-none font-mono focus:border-blue-500"
                              />
                            </td>
                            <td className="p-2">
                              <input
                                type="number"
                                placeholder="Rp"
                                className="w-full p-2 text-xs border border-slate-300 rounded outline-none focus:border-blue-500"
                              />
                            </td>
                            <td className="p-2">
                              <input
        ...
        <div className="px-6 py-4 border-t border-slate-200 bg-white flex justify-end gap-3 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-10">
          ...
          <button className="px-5 py-2.5 text-sm font-bold text-white bg-blue-600 rounded-lg shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all active:scale-95">
