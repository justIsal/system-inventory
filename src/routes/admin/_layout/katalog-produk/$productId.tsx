...
export const Route = createFileRoute('/admin/_layout/katalog-produk/$productId')({
...
function RouteComponent() {
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      ...
      try {
        ...
        setError(
          error.response?.data?.message || (err as Error).message || 'Gagal memuat data produk.',
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const handleUpdateProduct = async (data: unknown) => {
    ...
    try {
      ...
      console.error('Failed to update product:', errorObj);
      alert(errorObj.response?.data?.message || 'Gagal memperbarui produk.');
    } finally {
  ...
  const handleExportData = () => {
    ...
    exportToCSV(
      ...
      `Riwayat_Mutasi_${variant?.sku || productId}`,
      [
        {
          key: (m: unknown) =>
            new Date((m as { created_at: string }).created_at).toLocaleString('id-ID'),
          label: 'Waktu',
        },
        { key: 'type', label: 'Tipe' },
        {
          key: (m: unknown) => (m as { warehouse?: { name?: string } }).warehouse?.name || '-',
          label: 'Gudang',
        },
        { key: 'quantity', label: 'Jumlah' },
        { key: (m: unknown) => (m as { reference?: string }).reference || '-', label: 'Referensi' },
        {
          key: (m: unknown) => (m as { user?: { username?: string } }).user?.username || 'System',
          label: 'Oleh',
        },
      ],
    );
      ...
      <div className="h-full flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center bg-red-50 p-8 rounded-2xl border border-red-100 max-w-md text-center">
          ...
          <p className="text-sm text-red-600/80 mb-6">
            {error || 'Data yang Anda cari mungkin telah dihapus.'}
          </p>
          <button
            onClick={handleBack}
            className="px-5 py-2.5 text-sm font-bold text-white bg-slate-800 rounded-lg hover:bg-slate-900 transition-colors"
          >
            Kembali ke Katalog
          </button>
        </div>
    ...
    <>
      ...
      <div className="h-full flex flex-col font-sans space-y-8 animate-in slide-in-from-bottom-4 fade-in duration-500 pb-12 print:hidden">
        ...
        <div className="space-y-10 pt-4">
          ...
          <div>
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-4">
              <h3 className="text-[13px] font-black text-slate-500 uppercase tracking-widest">
                Riwayat Mutasi
              </h3>
              <div className="flex items-center gap-3">
                <Button
        ...
        <Modal
          ...
          onClose={() => {
            if (!isSubmitting) setIsEditModalOpen(false);
          }}
          title="Edit Produk"
      ...
      <div className="hidden print:block bg-white text-black font-sans w-full">
        ...
        <div className="mb-8">
          <h3 className="text-sm font-bold uppercase border-b border-slate-300 pb-2 mb-3">
            Sebaran Stok Per Gudang
          </h3>
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody>
              {variant?.movements && variant.movements.length > 0 ? (
                ...
                <tr>
                  <td colSpan={6} className="py-4 text-center italic text-slate-500">
                    Belum ada mutasi stok terdata.
                  </td>
                </tr>
              )}
            </tbody>
