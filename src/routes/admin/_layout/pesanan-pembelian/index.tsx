...
function PesananPembelianPage() {
  ...
  let filteredOrders = orders.filter((order) => {
    ...
    if (searchQuery) {
        ...
        matchesSearch = 
            `PO-${new Date(order.created_at).getFullYear()}-${String(order.po_id).padStart(3, '0')}`.toLowerCase().includes(query) ||
  ...
  const columns: ColumnDef<PurchaseOrder>[] = [
    {
      header: 'NO. PO',
      cell: (row) => `PO-${new Date(row.created_at).getFullYear()}-${String(row.po_id).padStart(3, '0')}`,
    ...
    {
      header: 'TANGGAL',
      cell: (row) => new Date(row.created_at).toLocaleString('id-ID', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }),
    ...
    },
    {
      header: 'TOTAL HARGA',
      cell: (row) => {
          const totalHarga = row.items?.reduce((sum, item) => sum + (item.qty_ordered * Number(item.variant?.price_buy || 0)), 0) || 0;
    ...
    },
    {
      ...
      cell: (row) => {
        ...
        return (
          <span className={`px-2.5 py-1 rounded-full text-[10px] font-black tracking-wider border ${badgeColor}`}>
            {row.status}
          </span>
        );
      },
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex-1 w-full">
            <CatalogToolbar 
                ...
                onAddClick={() => navigate({ to: '/admin/pesanan-pembelian/create' as any })}
