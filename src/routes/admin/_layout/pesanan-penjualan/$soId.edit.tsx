...
import type { CreateSalesOrderPayload, SalesOrder } from '@/types/sales-order.types';

export const Route = createFileRoute('/admin/_layout/pesanan-penjualan/$soId/edit')({
...
function EditSalesOrderPage() {
  ...
  // -- Extracted SO Data --
  const [originalSo, setOriginalSo] = useState<SalesOrder | null>(null);
  ...
  // Fetch Master Data AND Original SO
  const fetchAllData = async () => {
    if (!soId) return;

    try {
      setIsLoadingData(true);
      const [prodRes, wareData, soData] = await Promise.all([
        productService.getProducts(1, 100),
        getPublicWarehouses(),
        salesOrderService.getSalesOrderById(Number(soId)),
      ]);
      setProducts(prodRes.data);
      setWarehouses(wareData);
      setOriginalSo(soData);

      // Hydrate Form
      setCustomerName(soData.customer_name || '');
      setCustomerContact(soData.customer_contact || '');
      setShippingAddress(soData.shipping_address || '');
      setWarehouseId(soData.warehouse_id || 0);
      setOrderDate(new Date(soData.created_at).toISOString().split('T')[0]);
      ...
      if (soData.items && soData.items.length > 0) {
        setItems(
          soData.items.map((item) => ({
            ...
            price_sell: item.variant?.price_sell || 0,
          })),
        );
      } else {
        setItems([{ id: crypto.randomUUID(), variant_id: 0, qty_requested: 1, price_sell: 0 }]);
      }
    } catch (error) {
      console.error('Failed to load edit data:', error);
      setNotification({
        type: 'error',
        message: 'Gagal memuat data Produk atau Pesanan (Mungkin ID tidak ditemukan/Terhapus).',
      });
    } finally {
  ...
  const handleSubmit = async (status: 'DRAFT' | 'PENDING') => {
    ...
    try {
      ...
      const payload: Omit<CreateSalesOrderPayload, 'admin_id'> = {
      ...
      await salesOrderService.updateSalesOrder(soId, payload);
      setNotification({
        type: 'success',
        message: `Pesanan Penjualan #${soId} berhasil diperbarui!`,
      });
      ...
      setNotification({
        ...
        message:
          ...
          'Gagal memperbarui pesanan penjualan. Status SO (Contoh: SHIPPED) mungkin tidak bisa diedit.',
      });
    } finally {
  ...
  const isReadOnly = originalSo && originalSo.status !== 'DRAFT' && originalSo.status !== 'PENDING';
    ...
    <div className="mx-auto pb-10 animate-in fade-in duration-500">
          ...
          <div>
            ...
            {originalSo && (
              <p className="text-xs text-slate-500 mt-1 uppercase font-semibold">
                Status Saat Ini: <span className="text-indigo-600">{originalSo.status}</span>
              </p>
            )}
          </div>
        </div>
      ...
      {isReadOnly && (
        <div className="mb-4">
          <Alert variant="warning">
            Karena status pesanan ini sudah <b>{originalSo.status}</b>, Anda tidak dapat mengubah
            detail pelanggan atau produk. Mode Baca-Saja aktif.
          </Alert>
        </div>
      )}
      ...
      {isLoadingData ? (
        <div className="h-64 flex flex-col items-center justify-center bg-white rounded-xl shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] border border-slate-100">
          ...
          <p className="text-sm text-slate-500 font-medium">Memuat Data Pesanan...</p>
        </div>
      ) : (
        <div className="space-y-6">
          {/* SECTION 1: Informasi Pelanggan & Pengiriman */}
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="space-y-2 lg:col-span-1">
                ...
                <input
                  ...
                  className="w-full text-sm border-slate-200 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 py-2.5 px-3 bg-white disabled:bg-slate-100 disabled:opacity-75"
                  value={customerName}
              ...
              <div className="space-y-2 lg:col-span-1">
                ...
                <div className="relative">
                  ...
                  <input
                    type="date"
                    disabled
                    className="w-full text-sm border-slate-200 rounded-lg py-2.5 pl-3 pr-10 bg-slate-50 cursor-not-allowed opacity-75"
                    value={orderDate}
                  />
                </div>
              </div>
              ...
              <div className="space-y-2 lg:col-span-4">
                ...
                <textarea
                  disabled={isReadOnly || false}
                  className="w-full text-sm border-slate-200 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 py-2.5 px-3 bg-white min-h-[80px] disabled:bg-slate-100 disabled:opacity-75"
              ...
              {items.map((item) => (
                ...
                >
                  <div className="flex-1 min-w-[200px] space-y-2">
                    ...
                    <select
                      disabled={isReadOnly || false}
                      className="w-full text-sm border-slate-200 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 py-2.5 px-3 bg-white disabled:bg-slate-200/50"
                  ...
                  <div className="w-24 space-y-2">
                    <label className="text-xs font-bold text-slate-600">QTY KELUAR</label>
                    <input
                      type="number"
                      min="1"
                      disabled={isReadOnly || false}
                  ...
                  <div className="flex-1 min-w-[120px] space-y-2">
                    ...
                    <input
                      type="number"
                      disabled={isReadOnly || false}
                      className="w-full text-sm border-slate-200 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 py-2.5 px-3 bg-white text-right font-medium disabled:bg-slate-200/50"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-xl shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] border border-slate-100 overflow-hidden">
              ...
              <div className="p-6">
                <textarea
                  disabled={isReadOnly || false}
                  className="w-full text-sm border-slate-200 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 p-4 bg-white min-h-[128px] resize-y disabled:bg-slate-100"
      ...
      <Modal
        ...
        title="Konfirmasi Pembaruan SO"
        icon={<Package className="w-6 h-6 text-indigo-600" />}
        description="Harap periksa kembali pesanan penjualan (SO) yang di-update."
      >
        <div className="p-6 space-y-6">
          <div className="flex bg-slate-50 rounded-lg border border-slate-100 p-4 divide-x divide-slate-200">
            ...
            <div className="flex-1 px-4">
              <p className="text-xs text-slate-500 font-bold mb-1">GUDANG PENGIRIM</p>
          ...
          <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4 flex justify-between items-center">
            <div>
              ...
              <p className="text-xs text-indigo-600/80 mt-0.5">
                Akan dicatat sebagai {originalSo?.status === 'PENDING' ? 'PENDING' : 'DRAFT'} Sales
                Order.
              </p>
            </div>
